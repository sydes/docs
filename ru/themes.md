...
Каждая тема имеет хотя бы один макет, в идеале - по одному макету на каждый тип страниц сайта.
Благодаря ним, можно для каждой страницы делать уникальный дизайн или хотя бы структуру.

Каждый макет - это отдельный html файл с сеткой, инфоблоками и токенами данных.

Первая строка кода макета используется для хранения данных, таких как:

* @name(Страница) - человекочитаемое название макета, выводится в админке в списках выбора
* @extends(page) - название файла, который расширяет этот макет. Это может быть как оберточный файл в корне темы
(page, home-landing), так и другой макет, чью сетку хочется использовать, для избежания дублирования (layouts/article)

Далее идет html код макета.

<a name="display-data"></a>
## Вывод информации

Данные в оберточном файле и макетах выводятся через специальные метки в фигурных скобках, вида {some_token}.
...
`{type}` | тип страницы
`{created_at}` | Unix метка времени создания страницы
`{modified_at}` | Unix метка времени последнего редактирования страницы
`{title}` | заголовок страницы
`{content}` | текст страницы после него же
`{locale}` | текущая локаль страницы
`{h1}` | дефолтное поле, по умолчанию равно title, может использоваться внутри тега h1 вне блока content

Основные метки для шаблона:

`{language}` | текущий язык сайта
`{head}` | тайтл, мета-теги страницы, стили
`{template_path}` | путь до темы, вида /themes/theme-name
...
`{data:shop_email}` | информация из темы
...
По умолчанию страницы ошибок `404 Not Found`, `403 Forbidden` и `503 Maintenance Mode` имеют макет `page`. Для того,
что бы их оформить в другом стиле достаточно создать макеты `error404.html`, `error403.html` и `error503.html`
соответственно и по желанию указать им другой оберточный файл.
