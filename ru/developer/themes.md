...
Установить тему можно одним из трех способов.
...
3. Запросить с помощью консоли из репозитория композера командой `composer require vendor/theme-name`
...
**composer.json** - Не обязателен, используется для установки через композер.
...
(page, home-landing), так и другой макет, чью сетку хочется использовать, во избежание дублирования (layouts/article)
...
Так же это рекомендованный способ модифицирования публичных тем.

### Для чего нужно использовать дочерние темы?

* Использование дочерних тем может ускорить разработку, ведь создав один раз родительскую тему вы можете
переиспользовать ее функционал, не плодя код
* При обновлении публичной темы, ваши изменения за пределами custom.css и custom.js будут стерты. Используя дочернюю тему
